"""Ingest pipeline: discover -> dedup (sha exact, pHash near) -> upload -> embed -> record.
...
from piclocker.embeddings import get_embedding, get_model
from piclocker.facts import FileFacts, gather_facts
from piclocker.upload import create_and_upload_content
...
def load_dedup_maps(db):
    """phash -> content_id for every known content."""
    phash_map = {}
    for cid, sha, ph in db.execute(
        "SELECT id, sha256_hex, phash FROM CONTENT WHERE status IN ('UPLOADED', 'INDEXED', 'INDEXING')"
    ):
        phash_map[ph] = cid
    return phash_map
...
def embedded_image(image: Image.Image, content_id: int, db, force=False):
    if image is None:
    ...
    db.execute("UPDATE CONTENT SET status = 'INDEXING' WHERE id = ?", [content_id])
    db.commit()
    try:
        vec = get_embedding(image)
    except Exception as e:
        db.execute("UPDATE CONTENT SET status = 'ERROR' WHERE id = ?", [content_id])
        db.commit()
        raise Exception(f"Error while embedding image: {e}")

    vec = vec.astype("float32").tobytes()
    db.execute("UPDATE CONTENT SET status = 'INDEXED', embedding = ? WHERE id = ?", [vec, content_id])
    db.commit()
        ...
        if existing:
            ...
            if near is not None and policy == "discard":
                ...
                if near is not None:
                    ...
                    row = db.execute(
                        "SELECT dup_group FROM CONTENT WHERE id = ?", [matched_id]
                    ).fetchone()
                    matched_group = row[0] if row else None
                    if matched_group is None:
                        dup_group = matched_id
                        db.execute(
                            "UPDATE CONTENT SET dup_group = ? WHERE id = ?",
                            [dup_group, matched_id]
                        )
                    else:
                        dup_group = matched_group
                    db.execute(
...
def sync_folder(folder, title=None):
    ...
    with get_db(config.DB_PATH) as db:
        row = db.execute(
            "SELECT id FROM SYNCED_FOLDERS WHERE parent_folder_path = ?", [str(folder)]
        ).fetchone()
        if row is None:
            folder_id = db.execute(
                "INSERT INTO SYNCED_FOLDERS (parent_folder_path, title) VALUES (?, ?)",
        ...
        for image_path in walk_images(folder):
            known = db.execute(
                "SELECT disk_mtime FROM FILES WHERE local_path = ?", [str(image_path)]
            ).fetchone()
    ...
    with ThreadPoolExecutor(max_workers=config.UPLOAD_WORKERS) as executor:
        futures = {
            executor.submit(ingest_file, image_path, parent_id=folder_id, policy=config.NEAR_DUP_POLICY): image_path
